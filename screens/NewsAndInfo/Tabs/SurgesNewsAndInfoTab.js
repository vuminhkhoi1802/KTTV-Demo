...
export default class SurgesNewsAndInfoTab extends React.Component {
  ...
  componentDidMount () {
    url = `${apiURL}api/data_collection_support/surgesinformation/`
    return fetch(url)
  ...
  SearchFilterFunction (text) {
    const newData = this.arrayHolder.filter(function (item) {
      const itemData = item.surges_information_title
        ? item.surges_information_title.toUpperCase()
        : ''.toUpperCase()
  ...
  clickedItemText (clickedItem) {
    this.props.navigation.navigate('SurgeNewsAndInfoDetail', {
      item: clickedItem
    })
                ...
                >
                  <View style={AppTabStyle.container}>
                    <Text style={AppTabStyle.textStyle}>
                      {''}
                      {item.surges_information_title} - {item.area__area_name}
                      {''}
                    </Text>
                  </View>
                </TouchableOpacity>
