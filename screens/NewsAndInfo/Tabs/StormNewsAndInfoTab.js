...
export default class StormNewsAndInfoTab extends React.Component {
  ...
  componentDidMount () {
    url = apiURL + 'api/data_collection_support/storm_information/1/';
    return fetch (url)
  ...
  SearchFilterFunction (text) {
    const newData = this.arrayHolder.filter (function (item) {
      const itemData = item.storm_name_vi
        ? item.storm_name_vi.toUpperCase ()
        : ''.toUpperCase ();
  ...
  clickedItemText (clickedItem) {
    this.props.navigation.navigate ('StormNewsAndInfoDetail', {
      item: clickedItem,
    });
          ...
          <FlatList
            ...
            renderItem={({item}) => {
              ...
              return (
                <ListItem
                  leftIcon={icon}
                  title={item.storm_name_vi}
                  titleStyle={AppTabStyle.textStyle}
                  subtitle={`${item.storm_code_vi} - ${item.province__province_name} - ${item.status}`}
