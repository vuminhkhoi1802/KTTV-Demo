...
export default class FloodNewsAndInfoTab extends React.Component {
  ...
  componentDidMount () {
    url = `${apiURL}api/data_collection_support/floodsinformation/`
    return fetch(url)
  ...
  clickedItemText (clickedItem) {
    this.props.navigation.navigate('FloodNewsAndInfoDetail', {
      item: clickedItem
    })
                ...
                >
                  <View style={AppTabStyle.container}>
                    <Text style={AppTabStyle.textStyle}>
                      {''}
                      {item.flood_name} - {item.area__area_name}
                      {''}
                    </Text>
                  </View>
                </TouchableOpacity>
